...
/**
 * Criteria class for the {@link com.visa.apply.domain.DMaikaRow} entity. This class is used
 * in {@link com.visa.apply.web.rest.DMaikaRowResource} to receive all the possible filtering options from
 * the Http GET request parameters.
 * For example the following could be a valid request:
 * {@code /d-maika-rows?id.greaterThan=5&attr1.contains=something&attr2.specified=false}
 * As Spring is unable to properly convert the types, unless specific {@link Filter} class are used, we need to use
 * fix type specific filters.
 */
@ParameterObject
@SuppressWarnings("common-java:DuplicatedBlocks")
public class DMaikaRowCriteria implements Serializable, Criteria {
    ...
    public DMaikaRowCriteria() {}

    public DMaikaRowCriteria(DMaikaRowCriteria other) {
        ...
        this.dcDarj = other.optionalDcDarj().map(StringFilter::copy).orElse(null);
    ...
    @Override
    public DMaikaRowCriteria copy() {
        return new DMaikaRowCriteria(this);
    }
    ...
    public StringFilter getDcDarj() {
        return dcDarj;
    }

    public Optional<StringFilter> optionalDcDarj() {
        return Optional.ofNullable(dcDarj);
    }

    public StringFilter dcDarj() {
        if (dcDarj == null) {
            setDcDarj(new StringFilter());
        }
        return dcDarj;
    }

    public void setDcDarj(StringFilter dcDarj) {
        this.dcDarj = dcDarj;
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(id, dcFamil, dcImena, dcPol, dcDarj, dcGrad, dcUlica, distinct);
    }

    // prettier-ignore
    @Override
    public String toString() {
        return "DMaikaRowCriteria{" +
            optionalId().map(f -> "id=" + f + ", ").orElse("") +
            optionalDcFamil().map(f -> "dcFamil=" + f + ", ").orElse("") +
            optionalDcImena().map(f -> "dcImena=" + f + ", ").orElse("") +
            optionalDcPol().map(f -> "dcPol=" + f + ", ").orElse("") +
            optionalDcDarj().map(f -> "dcDarj=" + f + ", ").orElse("") +
