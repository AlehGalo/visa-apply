...
public class DDomakinRowGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all dDomakinRows").get("/api/d-domakin-rows").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dDomakinRow")
                        .post("/api/d-domakin-rows")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"nomPok\": \"SAMPLE_TEXT\"" +
                                ", \"domGraj\": \"SAMPLE_TEXT\"" +
                                ", \"domFamil\": \"SAMPLE_TEXT\"" +
                                ", \"domIme\": \"SAMPLE_TEXT\"" +
                                ", \"domDarj\": \"SAMPLE_TEXT\"" +
                                ", \"domNm\": 0" +
                                ", \"domAdres\": \"SAMPLE_TEXT\"" +
                                ", \"vedDarj\": \"SAMPLE_TEXT\"" +
                                ", \"vedNm\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dDomakinRow_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created dDomakinRow").get("${new_dDomakinRow_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created dDomakinRow").delete("${new_dDomakinRow_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DDomakinRow entity").exec(scn);
