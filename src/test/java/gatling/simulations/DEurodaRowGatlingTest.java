...
public class DEurodaRowGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all dEurodaRows").get("/api/d-euroda-rows").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dEurodaRow")
                        .post("/api/d-euroda-rows")
                        ...
                        .body(
                            StringBody(
                                ...
                                "\"euFamil\": \"SAMPLE_TEXT\"" +
                                ", \"euImena\": \"SAMPLE_TEXT\"" +
                                ", \"euNacBel\": \"SAMPLE_TEXT\"" +
                                ", \"euRodstvo\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dEurodaRow_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created dEurodaRow").get("${new_dEurodaRow_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created dEurodaRow").delete("${new_dEurodaRow_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DEurodaRow entity").exec(scn);
