...
public class DFingersRowGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all dFingersRows").get("/api/d-fingers-rows").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dFingersRow")
                        .post("/api/d-fingers-rows")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"fnDatreg\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"fnDatvav\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"fnUsera\": \"SAMPLE_TEXT\"" +
                                ", \"fnSid\": 0" +
                                ", \"fnPnr\": 0" +
                                ", \"fnVidmol\": \"SAMPLE_TEXT\"" +
                                ", \"fnDrugi\": \"SAMPLE_TEXT\"" +
                                ", \"fnDeviceid\": 0" +
                                ", \"fnScanres\": 0" +
                                ", \"fnWidth\": 0" +
                                ...
                                ", \"fnPixeldepth\": 0" +
                                ", \"fnCompressalgo\": 0" +
                                ", \"fnFingerposition\": \"SAMPLE_TEXT\"" +
                                ", \"fnImagequality\": 0" +
                                ", \"fnImage\": \"SAMPLE_TEXT\"" +
                                ", \"fnImglen\": 0" +
                                ", \"fnNottakenreason\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dFingersRow_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created dFingersRow").get("${new_dFingersRow_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created dFingersRow").delete("${new_dFingersRow_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DFingersRow entity").exec(scn);
