...
public class DLcdopRowGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all dLcdopRows").get("/api/d-lcdop-rows").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dLcdopRow")
                        .post("/api/d-lcdop-rows")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"ldMrjdarj\": \"SAMPLE_TEXT\"" +
                                ", \"ldMrjnm\": \"SAMPLE_TEXT\"" +
                                ", \"ldMrjgraj\": \"SAMPLE_TEXT\"" +
                                ", \"ldZenen\": \"SAMPLE_TEXT\"" +
                                ", \"ldJitDarj\": \"SAMPLE_TEXT\"" +
                                ", \"ldJitNm\": \"SAMPLE_TEXT\"" +
                                ", \"ldJitUl\": \"SAMPLE_TEXT\"" +
                                ", \"ldTel\": 0" +
                                ", \"ldRabota\": \"SAMPLE_TEXT\"" +
                                ", \"ldProfkod\": \"SAMPLE_TEXT\"" +
                                ", \"ldProfesia\": \"SAMPLE_TEXT\"" +
                                ", \"ldSlDarj\": \"SAMPLE_TEXT\"" +
                                ", \"ldSlNm\": \"SAMPLE_TEXT\"" +
                                ", \"ldSlUl\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dLcdopRow_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created dLcdopRow").get("${new_dLcdopRow_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created dLcdopRow").delete("${new_dLcdopRow_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DLcdopRow entity").exec(scn);
