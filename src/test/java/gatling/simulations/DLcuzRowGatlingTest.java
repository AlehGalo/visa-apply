...
public class DLcuzRowGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all dLcuzRows").get("/api/d-lcuz-rows").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dLcuzRow")
                        .post("/api/d-lcuz-rows")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"nacBel\": \"SAMPLE_TEXT\"" +
                                ", \"nacPasp\": 0" +
                                ", \"paspVal\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"graj\": \"SAMPLE_TEXT\"" +
                                ", \"famil\": \"SAMPLE_TEXT\"" +
                                ", \"imena\": \"SAMPLE_TEXT\"" +
                                ", \"datRaj\": \"SAMPLE_TEXT\"" +
                                ", \"pol\": \"SAMPLE_TEXT\"" +
                                ", \"datIzd\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dLcuzRow_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created dLcuzRow").get("${new_dLcuzRow_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created dLcuzRow").delete("${new_dLcuzRow_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DLcuzRow entity").exec(scn);
