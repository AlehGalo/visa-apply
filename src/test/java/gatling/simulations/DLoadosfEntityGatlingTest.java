...
public class DLoadosfEntityGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all dLoadosfEntities").get("/api/d-loadosf-entities").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dLoadosfEntity")
                        .post("/api/d-loadosf-entities")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"version\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dLoadosfEntity_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created dLoadosfEntity").get("${new_dLoadosfEntity_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created dLoadosfEntity").delete("${new_dLoadosfEntity_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DLoadosfEntity entity").exec(scn);
