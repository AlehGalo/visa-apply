...
public class DMaikaRowGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all dMaikaRows").get("/api/d-maika-rows").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dMaikaRow")
                        .post("/api/d-maika-rows")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dMaikaRow_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created dMaikaRow").get("${new_dMaikaRow_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created dMaikaRow").delete("${new_dMaikaRow_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DMaikaRow entity").exec(scn);
