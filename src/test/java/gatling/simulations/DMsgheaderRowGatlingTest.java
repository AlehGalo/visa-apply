...
public class DMsgheaderRowGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all dMsgheaderRows").get("/api/d-msgheader-rows").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dMsgheaderRow")
                        .post("/api/d-msgheader-rows")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"mhKscreated\": \"SAMPLE_TEXT\"" +
                                ", \"mhRegnom\": 0" +
                                ", \"mhVfsrefno\": \"SAMPLE_TEXT\"" +
                                ", \"mhUsera\": \"SAMPLE_TEXT\"" +
                                ", \"mhDatvav\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dMsgheaderRow_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created dMsgheaderRow").get("${new_dMsgheaderRow_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created dMsgheaderRow").delete("${new_dMsgheaderRow_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DMsgheaderRow entity").exec(scn);
