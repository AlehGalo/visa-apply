...
public class DSaprugaRowGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all dSaprugaRows").get("/api/d-sapruga-rows").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dSaprugaRow")
                        .post("/api/d-sapruga-rows")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"spMrjdarj\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dSaprugaRow_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created dSaprugaRow").get("${new_dSaprugaRow_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created dSaprugaRow").delete("${new_dSaprugaRow_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DSaprugaRow entity").exec(scn);
