...
public class DVoitRowGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all dVoitRows").get("/api/d-voit-rows").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new dVoitRow")
                        .post("/api/d-voit-rows")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_dVoitRow_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created dVoitRow").get("${new_dVoitRow_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created dVoitRow").delete("${new_dVoitRow_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DVoitRow entity").exec(scn);
